...
def compute_relative_l2_norm(output1: np.ndarray, output2: np.ndarray) -> float:
    ...
    if output1.shape != output2.shape:
        print(f"Warning: Shape mismatch in L2 computation: {output1.shape} vs {output2.shape}")
        return float('nan')

    # Check for NaN or Inf values in inputs
    if np.isnan(output1).any() or np.isnan(output2).any():
        print(f"Warning: NaN values detected in outputs")
        return float('nan')

    if np.isinf(output1).any() or np.isinf(output2).any():
        print(f"Warning: Inf values detected in outputs")
        return float('nan')
...
def upgrade_quantization_to_uint16(model: onnx.ModelProto, tensor_name: str) -> bool:
    ...
    Note: This changes the tensor type to UINT16 but keeps the scale unchanged.
    The quantized values remain in [0, 255] range (stored as uint16), which allows
    ONNX Runtime to use optimized uint16×uint8 kernels while maintaining accuracy.
    """
    modified = False
    ...
    # No need to modify DequantizeLinear nodes - the scale remains valid for the [0, 255] range
